...
documentRouter.post("/", validateToken_1.validateToken, async (req, res) => {
    try {
        const newDocument = await Document_1.DocumentModel.create({
...
documentRouter.get("/", validateToken_1.validateToken, async (req, res) => {
    try {
...
documentRouter.get("/public/:id", async (req, res) => {
    try {
        const document = await Document_1.DocumentModel.findOne({ _id: req.params.id });
        if (!document) {
...
documentRouter.get("/:id", validateToken_1.validateToken, async (req, res) => {
    try {
        // If the document exists and is currently locked, block access
        const existingDoc = await Document_1.DocumentModel.findById(req.params.id);
        ...
        const document = await Document_1.DocumentModel.findOneAndUpdate({
            _id: req.params.id,
            $or: [
                { ownerId: req.user._id },
                { sharedWith: req.user._id }
...
documentRouter.put("/:id", validateToken_1.validateToken, async (req, res) => {
    try {
...
documentRouter.patch("/:id/unlock", validateToken_1.validateToken, async (req, res) => {
    try {
        const document = await Document_1.DocumentModel.findOneAndUpdate({
            _id: req.params.id,
            $or: [
        ...
        }, { $set: { isEditing: false } }, { returnDocument: 'after' });
        if (!document) {
            return res.status(404).json({ message: 'Document not found' });
        }
...
documentRouter.delete("/:id", validateToken_1.validateToken, async (req, res) => {
    try {
        const document = await Document_1.DocumentModel.findOne({ _id: req.params.id });
        if (!document) {
            return res.status(404).json({ message: 'Document not found' });
        ...
        if (document.ownerId.toString() === req.user._id.toString()) {
            ...
            await Document_1.DocumentModel.findByIdAndDelete(req.params.id);
            return res.status(200).json({ message: 'Document deleted permanently' });
        }
        else {
            // Shared user: Remove from sharedWith array
            await Document_1.DocumentModel.findByIdAndUpdate(req.params.id, { $pull: { sharedWith: req.user._id } });
...
documentRouter.post("/:id/share", validateToken_1.validateToken, async (req, res) => {
    try {
